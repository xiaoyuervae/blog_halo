...
<#if options.theme_material_scheme?if_exists == "Isolation">
    <div class="post-header_info without-thumbnail">
        <!-- Author Avatar & Name -->
        <img src="${user.userAvatar?default('/material/source/img/avatar.png')}" class="avatar-img" width="44px" height="44px" alt="${user.userDisplayName?default('blog_halo')}'s avatar">
        <span class="name-span">${user.userDisplayName?default('blog_halo')}</span>
    </div>
</#if>